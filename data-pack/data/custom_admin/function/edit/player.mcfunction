# Eylem Hedef Oyuncusu Değiştir (MAKRO) — {id:"...", player:"@s"}
$execute unless data storage mc:handler data.actions[{id:"$(id)"}] run tellraw @s ["",{"text":"[GULCE] ","color":"red","bold":true},{"text":"Hata: Eylem bulunamadı — ","color":"red"},{"text":"$(id)","color":"yellow"}]
$execute unless data storage mc:handler data.actions[{id:"$(id)"}] run return 0
$data modify storage mc:handler data.actions[{id:"$(id)"}].player set value "$(player)"
$tellraw @s ["",{"text":"[GULCE] ","color":"gold","bold":true},{"text":"Eylem hedefi güncellendi: ","color":"green"},{"text":"$(id)","color":"yellow"},{"text":" → ","color":"gray"},{"text":"$(player)","color":"aqua"}]
